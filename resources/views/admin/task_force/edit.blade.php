...
@section('header-title', 'Edit Task Force Member')
@section('header-subtitle', 'Edit details for ' . $member->name)

@section('content')

<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
        <form method="POST" action="{{ route('admin.task-force.update', $member) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $member->email) }}" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-ans-dark-green/20 focus:border-ans-dark-green outline-none transition-all">
