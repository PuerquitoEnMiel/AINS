    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
        <form method="POST" action="{{ route('admin.prompt-tips.update', $prompt) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="target_role" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-ans-dark-green/20 focus:border-ans-dark-green outline-none transition-all bg-white">
                        <option value="docentes" {{ old('target_role', $prompt->target_role) == 'docentes' ? 'selected' : '' }}>Teachers</option>
                        <option value="estudiantes" {{ old('target_role', $prompt->target_role) == 'estudiantes' ? 'selected' : '' }}>Students</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="complexity" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-ans-dark-green/20 focus:border-ans-dark-green outline-none transition-all bg-white">
                        <option value="Básico" {{ old('complexity', $prompt->complexity) == 'Básico' ? 'selected' : '' }}>Basic</option>
                        <option value="Intermedio" {{ old('complexity', $prompt->complexity) == 'Intermedio' ? 'selected' : '' }}>Intermediate</option>
                        <option value="Avanzado" {{ old('complexity', $prompt->complexity) == 'Avanzado' ? 'selected' : '' }}>Advanced</option>
                    </select>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="px-6 py-2.5 bg-ans-dark-green text-white rounded-xl font-semibold hover:bg-ans-seal-green transition-all shadow-md text-sm">Update Prompt</button>
