...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
        ...
        <a href="{{ $badge->certification_url }}" target="_blank" class="mt-4 flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 font-medium">
            ...
            Visitar programa de certificación oficial
        </a>
        @endif
    </div>
    ...
    <div class="mb-6 p-4 rounded-2xl border
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="font-semibold text-sm text-gray-800">Estado: {{ $existing->statusLabel() }}</p>
                @if($existing->admin_notes)
                <p class="text-sm text-gray-600 mt-1">Nota del admin: "{{ $existing->admin_notes }}"</p>
                @endif
                @if($existing->isApproved() && $existing->expires_at)
                <p class="text-xs text-green-700 mt-1">Tu insignia vence el {{ $existing->expires_at->format('d/m/Y') }}</p>
                @endif
            </div>
        </div>
        @if($existing->isRejected())
        <p class="text-xs text-red-700 mt-2 font-medium">Puedes volver a subir tu evidencia corregida abajo.</p>
        @elseif($existing->isPending())
        <p class="text-xs text-amber-700 mt-2">El administrador revisará tu evidencia pronto. Puedes actualizar tu envío si necesitas.</p>
        @endif
    </div>
    @endif
    ...
    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-2xl">
        <p class="text-sm font-semibold text-blue-800 mb-1">📋 Instrucciones</p>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('badge-evidence.store', $badge) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <!-- Certificate URL -->
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">URL del Certificado <span class="text-gray-400 font-normal">(Credly, Google, etc.)</span></label>
                ...
                <p class="text-xs text-gray-400 mt-1">Pega el enlace público de tu certificado digital (Credly, Google Certificates, LinkedIn, etc.)</p>
            </div>

            <!-- File Upload -->
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Archivo de Evidencia <span class="text-gray-400 font-normal">(PDF, JPG, PNG — máx. 5MB)</span></label>
                <div id="file-drop-zone" class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-ans-dark-green/40 transition-colors cursor-pointer"
                    ...
                    <p class="text-sm text-gray-500" id="file-label">Arrastra un archivo o haz clic para seleccionar</p>
                    <p class="text-xs text-gray-400 mt-1">PDF, JPG, PNG hasta 5MB</p>
                </div>
                <input type="file" name="evidence_file" id="evidence_file" accept=".pdf,.jpg,.jpeg,.png" class="hidden"
                       onchange="document.getElementById('file-label').textContent = this.files[0]?.name ?? 'Arrastra un archivo o haz clic para seleccionar'">
            </div>

            <!-- Notes -->
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Notas adicionales <span class="text-gray-400 font-normal">(opcional)</span></label>
                <textarea name="notes" rows="3" placeholder="Cuéntanos cuándo y cómo obtuviste esta certificación..." class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-ans-dark-green/20 focus:border-ans-dark-green outline-none transition-all">{{ old('notes', $existing?->notes) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                ...
                <a href="{{ route('badges.show', $badge->slug) }}" class="px-6 py-3 bg-gray-100 text-gray-600 rounded-xl font-semibold hover:bg-gray-200 transition-all text-sm">Cancelar</a>
            </div>
        </form>
    </div>
    @endif
